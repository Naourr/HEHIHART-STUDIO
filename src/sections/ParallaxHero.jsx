...
export default function ParallaxHero({
  ...
  const BASE = "/art/landing/";
  const COUNT = 12;

  // respect OS only if forceMotion is false
  ...
  // bottom (0) => 0px; top (1) => depthPx.max
  ...
  return (
    ...
    >
      <div
        ref={wrapRef}
        className="relative w-full h-full overflow-hidden bg-neutral-100 dark:bg-[#0B0B0C] rounded-none sm:rounded-2xl"
        style={{ perspective: "1000px", transformStyle: "preserve-3d" }}
      >
        {/* NOTE: removed global tilt — we tilt each layer by depth instead */}
        <div className="absolute inset-0">
          {layers.map((layer) => {
            const s = strengthFor(layer.depthNorm);

            // per-layer parallax translations (scaled by depth)
            const tx = useTransform(x, (v) => (DISABLE_MOTION ? 0 : v * s));
            const ty = useTransform(y, (v) => (DISABLE_MOTION ? 0 : v * s));

            // per-layer tilt (scaled by depth) — bottom gets 0deg
            ...
            // subtle per-layer roll (rotateZ), also depth-scaled
            ...
            return (
              <motion.img
                ...
                style={{
                  ...
                  rotateY: rY,
                  rotateZ: rZ,
                  zIndex: layer.z, // 1..COUNT (topmost on top)
                  scale: baseScale,
                }}
                initial={{ opacity: 0 }}
